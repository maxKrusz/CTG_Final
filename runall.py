...
class Worker(threading.Thread):
    ...
    def run(self):
        while self.tp.more_tasks():
            ...
            os.system(f"CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES={self.gpu} " + cmd.format(self.id))
            t2 = time.time()
                ...
                for threshold in (schedule["run"][attack]["thresholds"]):
                    ...
                    for noise_type in schedule["run"][attack]["datasets"][dataset]["noise"].keys():
                        ...
                        for noise_level in reversed(schedule["run"][attack]["datasets"][dataset]["noise"][noise_type]):
                            ...
                            pending_task = f"python3 main.py -a {attack} -n {n_images} -d {dataset} --seed {args.seed} --arch {arch} -t {threshold} -D {noise_type} --noise_level {noise_level} -o {exp_folder} -eps {epsilon} -q {query_budget} -tid {{}}"
